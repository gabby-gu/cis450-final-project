...
const author = async function(req, res) {
  ...
  const name = 'Sebin Lee';
  const pennKey = 'seblee';
...
const top_songs = async function(req, res) {
  ...
  if (!page) {
    ...
    //res.json([]); // replace this with your implementation
    connection.query(`
    SELECT song_id, Songs.title, album_id, Albums.title as album, plays
    FROM Songs
...
const top_albums = async function(req, res) {
  ...
  if (!page) {
    connection.query(`
    SELECT album_id, Albums.title, sum(plays) as plays
    FROM Songs
    LEFT JOIN Albums USING (album_id)
    GROUP BY album_id
...
const search_songs = async function(req, res) {
  ...
  const explicit = req.query.explicit !== 'true' | !req.query.explicit ? 0 : 1;
